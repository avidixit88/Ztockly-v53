...
def _session_liquidity_levels(df: pd.DataFrame, interval_mins: int, orb_minutes: int):
    """Compute prior session H/L, current premarket H/L, and ORB levels (simple approximation)."""
    ...
    dates = sorted({x.date() for x in d["_ts"]})
    prev_date = dates[-2] if len(dates) >= 2 else cur_date

    def _is_pre(t): return (t.time() >= pd.Timestamp("04:00", tz="America/New_York").time()) and (t.time() < pd.Timestamp("09:30", tz="America/New_York").time())
    def _is_rth(t): return (t.time() >= pd.Timestamp("09:30", tz="America/New_York").time()) and (t.time() <= pd.Timestamp("16:00", tz="America/New_York").time())

    prev = d[d["_ts"].dt.date == prev_date]
    prev_rth = prev[prev["_ts"].apply(_is_rth)]
    prior_high = float(prev_rth["high"].max()) if len(prev_rth) else float(prev["high"].max())
    prior_low = float(prev_rth["low"].min()) if len(prev_rth) else float(prev["low"].min())

    cur = d[d["_ts"].dt.date == cur_date]
    cur_pre = cur[cur["_ts"].apply(_is_pre)]
    pre_hi = float(cur_pre["high"].max()) if len(cur_pre) else None
    pre_lo = float(cur_pre["low"].min()) if len(cur_pre) else None

    cur_rth = cur[cur["_ts"].apply(_is_rth)]
    orb_bars = max(1, int(math.ceil(float(orb_minutes) / float(interval_mins))))
